...
def save_curve_csv(curve_dict: Dict[str, Dict[str, float]], csv_path: str, key_name="key"):
    """
    curve_dict: {key_str: {MAE,RMSE,R2,R}}
    key_name: Name of the first CSV column, e.g. "train_ratio" or "support".
    """
...
class TaskDataset:
    """
    Dataset wrapper for one system/task.
    Stores the full dataset and a fixed train/val/test split used for sampling
    support-query episodes during meta-training and evaluation.
    """
    def __init__(self, X: np.ndarray, y: np.ndarray, name: str, seed: int = 42):
        ...
        # Keep training-set target statistics for reporting and R2 reference.
        y_tr = self.y[self.tr_idx]
        self.mu = float(np.mean(y_tr))
        self.sd = float(np.std(y_tr) + 1e-8)
        print(f"[{self.name}] y mean={self.mu:.3f}, std={self.sd:.3e}, min={self.y.min():.3f}, max={self.y.max():.3f}")
    ...
    def sample_support_query(self, split="train", shots=16, queries=64):
        """
        Sample a support/query episode from the requested split and return
        Xs, ys, Xq, yq without any target normalization.
        """
        pool = {"train": self.tr_idx, "val": self.va_idx, "test": self.te_idx, "all": self.all_idx}[split]
...
def loso_partitions(tasks: List[TaskDataset], seed: int = 42):
    """LOSO: hold out one system as meta-test; split remaining systems into meta-train/meta-val."""
    out = []
    K = len(tasks)
    rng = np.random.RandomState(seed)
    for i in range(K):
...
class MAMLTrainer:
    ...
    def _clone_model(self):
        # Create a task-specific copy of the current meta-model for inner-loop adaptation.
    ...
    def meta_train(self, tasks_train, tasks_val,
        ...
        for ep in range(1, episodes + 1):
            # Sample one task, adapt on its support set, then update the meta-model
            # using the query loss from the adapted copy.
            task = tasks_train[int(rng.choice(len(tasks_train), size=1)[0])]
            Xs, ys, Xq, yq = task.sample_support_query("all", shots, queries)
    ...
    def evaluate_fewshot_curve(self, tasks, support_sizes=(5,10,20,30,40,50),
        ...
        """
        Evaluate few-shot performance for each support size in support_sizes.
        Returns a dictionary of the form {size_str: metrics}.
        """
        out = {}
    ...
    def evaluate_full(self, task, inner_steps=5,
                    ...
                    seed=42):
        """
        Full-task meta-testing:
        For a held-out task, randomly split the entire dataset into
        support and query sets according to a given ratio.
        No y normalization is applied.
        """
        rng = np.random.RandomState(seed)
        results = {}

        # Use the full held-out task dataset and resample support/query splits by ratio.
        X_all = task.X
        y_all = task.y
        N = len(y_all)
        ...
        for ratio in train_ratios:
            n_support = max(2, int(N * ratio))
            perm = rng.permutation(indices)

            s_idx = perm[:n_support]
            q_idx = perm[n_support:]

            Xs, ys = X_all[s_idx], y_all[s_idx]
            Xq, yq = X_all[q_idx], y_all[q_idx]

            # Convert the sampled arrays to torch tensors on the target device.
            Xs_t = torch.from_numpy(Xs).float().to(self.device)
            ys_t = torch.from_numpy(ys).float().to(self.device)
            Xq_t = torch.from_numpy(Xq).float().to(self.device)

            # Adapt a cloned model on the support set, then predict on the query set.
            fast = self._clone_model()
            opt = torch.optim.SGD(fast.parameters(), lr=self.inner_lr)
            ...
            print(
                f"[{task.name}] Full-task ratio={ratio:.2f} | "
                f"MAE={mae:.4f}, RMSE={rmse:.4f}, R2={r2:.4f}, R={r:.4f}"
            )
    ...
    def evaluate_clustered(self, task, train_ratio=0.1, n_clusters=None, inner_steps=5, seed=42):
        """Use KMeans prototype points as support and evaluate on the remaining samples."""
        from sklearn.cluster import KMeans
        from scipy.spatial.distance import cdist

        X = task.X; y = task.y; N = len(y)
        if n_clusters is None: n_clusters = max(1, int(train_ratio * N))
        print(f"[{task.name}] Cluster support size = {n_clusters}/{N} (ratio={train_ratio:.2f})")
...
# =========================================================
# K-fold CV (meta-train systems) + Refit
# =========================================================
...
def main():
    ...
    ap.add_argument("--episodes", type=int, default=2000)      # Number of meta-training episodes used for each fold refit.
    ap.add_argument("--shots", type=int, default=50)
    ap.add_argument("--queries", type=int, default=100)
    ap.add_argument("--eval_episodes", type=int, default=50)   # Number of repeated episodes used during evaluation.
    ...
    ap.add_argument("--cluster_eval", action="store_true", help="whether to run clustered evaluation")
    ap.add_argument("--fewshot_list", type=str, default="5,10,20,30,40,50,75,100", help="few-shot support sizes, comma-separated")
    ap.add_argument("--full_curve", type=str, default="0.04,0.08,0.12,0.16,0.20,0.3,0.5", help="full-data train ratios, comma-separated")
    ap.add_argument("--cluster_curve", type=str, default="0.04,0.08,0.12,0.16,0.20,0.3,0.5", help="clustered train ratios, comma-separated")
    ap.add_argument("--out_dir", type=str, default="runs_maml_cv_v3")
    args = ap.parse_args()
    ...
    # Parse list-like CLI arguments and save the resolved run configuration.
    ...
    # Load all systems and infer the input feature dimension.
    ...
    for k, (meta_train_init, meta_val_init, test_task) in enumerate(folds, 1):
        ...
        # --------- (1) Build one refitted meta-model for the current LOSO fold. ---------
        # --------- (2) Train on all non-held-out systems without a validation split. ---------
        trainer = MAMLTrainer(in_dim, hidden=args.hidden_tuple,
                              dropout=args.dropout, inner_lr=args.inner_lr, meta_lr=args.meta_lr)
        trainer.meta_train(remain_tasks, tasks_val=[],   # Refit directly on all remaining systems.
                           episodes=args.episodes,
                           shots=args.shots, queries=args.queries,
                           inner_steps=args.inner_steps,
                           val_every=0, patience=args.patience, seed=args.seed, verbose=True)

        # --------- (3) Meta-test the held-out system with multiple evaluation protocols. ---------
        ...
        # --------- (4) Save metrics and the refitted model for this fold. ---------
